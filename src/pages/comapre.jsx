...
import BidModal from '../component/BidModal.jsx';
...
function Compare() {
    ...
    useEffect(() => {
        ...
        if (titles.length > 0) {
            Promise.all(titles.map((title) => axios.get(`http://localhost:3000/api/items/${encodeURIComponent(title)}`)))
                .then((responses) => {
        ...
        <div className="font-inter">
            <div className="bg-red-600 text-white text-sm font-semibold py-2 px-4">
                <div className="max-w-screen-xl mx-auto flex justify-between items-center px-4 py-2">
                    ...
                    <button className="text-white font-semibold px-4 py-1 rounded hover:underline">List an item</button>
                </div>
            </div>

            <div className="max-w-screen-xl mx-auto px-4 py-6">
                <div className="flex items-center justify-between mb-4">
                    <h1 className="text-xl font-semibold text-[#000000]">Compare listings</h1>
                    <div className="flex space-x-2">
                        <button className="flex items-center mx-[30px]" style={{ color: '#3E74CB' }} onClick={() => navigate('/search')}>
                            <img src="/back search.png" alt="Remove" className="w-[40pxpx] h-[40px] hover:text-blue-500 mx-1" />
                            Back to search
                        </button>
                        <button
                            style={{ color: '#3E74CB' }}
                    ...
                    {items.map((item, index) => (
                        <div key={index} className="w-full md:w-1/2 lg:w-1/2 p-2">
                            <div className=" bg-white overflow-hidden relative">
                                ...
                                >
                                    <img src="/remove.png" alt="Remove" className="w-[40pxpx] h-[40px]  hover:text-blue-500" />
                                </button>

                                <div className="h-[500px] flex items-center justify-center">
                                    <img src={item.image || '/api/placeholder/400/320'} alt={item.title} className="max-h-full max-w-full object-cover " />
                                </div>

                                <div className="p-4" style={{ color: '#76716D' }}>
                                    <p className="text-sm mb-1">{item.location || 'Auckland City, Auckland'}</p>
                                    <h2 className="text-lg font-medium mb-2 text-black">{item.title || 'Coffee Table wooden'}</h2>

                                    <div className="flex items-center text-sm mb-3">
                                        <img src="/view1.png" alt="Remove" className="w-[20px] h-[20px] hover:text-blue-500" />
                                    ...
                                    <div className="text-sm mb-3 flex justify-between">
                                        <div className="flex items-center">
                                            <img src="eye.png" alt="eye" className="w-[20px] h-[20px] hover:text-blue-500" />

                                            <span>{item.views || '177'} Views</span>
                                        </div>

                                        <div className="flex items-center">
                                            <img src="Group.png" alt="watchlist" className="w-[15px] h-[15px] hover:text-blue-500" />

                                            <span>{item.watchlisted || '15'} others watchlisted</span>
                                        </div>

                                        <button className="flex items-center" style={{ color: '#3E74CB' }}>
                                            <img src="/share.png" alt="share" className="w-[20px] h-[20px] hover:text-blue-500" />
                                            Share
                                        </button>
                                    </div>
                                    ...
                                    <div className="mb-4">
                                        ...
                                        <Link to="/search/payment">
                                            <button className="w-full py-2 rounded flex items-center justify-center mb-2" style={{ backgroundColor: '#3E74CB', color: 'white' }}>
                                                Buy Now
                                            </button>
                                        </Link>
                                    ...
                                    <div className="border-t pt-4">
                                        ...
                                        <div className="mb-3">
                                            <p className="text-sm">
                                                Your bid: <span className="font-medium">${item.your_bid || '45.00'}</span>
                                            </p>
                                    ...
                                    <div className="border-t mt-4 pt-4 px-4 pb-6">
                                        <div className="border rounded-lg p-4 text-center">
                                            <img src="/api/placeholder/400/320" alt="Seller profile" className="w-16 h-16 mx-auto rounded-full object-cover mb-2" />
            ...
            {isBidModalOpen && selectedItem && <BidModal item={selectedItem} onClose={() => setIsBidModalOpen(false)} />}
        </div>
    );
}
